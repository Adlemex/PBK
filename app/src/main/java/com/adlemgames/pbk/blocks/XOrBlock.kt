...
class XOrBlock(view: View, id: String, interfaces: BlocksInterface) : Block(view, id, interfaces) {
    ...
    fun calc(){
        ...
        if (connects.size == 2){
            ...
            cur_state = two?.let { one?.xor(it) } == true
            out_text.text = two?.let { one?.xor(it)?.toInt() }.toString()
        }
        else {
    ...
    companion object {
        const val ID = R.layout.block_exclude_or
    }
}