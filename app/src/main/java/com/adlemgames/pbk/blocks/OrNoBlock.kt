...
class OrNoBlock(view: View, id: String, interfaces: BlocksInterface) : Block(view, id, interfaces) {
    ...
    fun calc(){
        ...
        if (connects.size == 2){
            ...
            cur_state = two?.let { one?.or(it) }  == false
            out_text.text = two?.let { one?.or(it) == false }?.toInt().toString()
        }
        else {
